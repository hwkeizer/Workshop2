...
public class AccountDaoMysqlTest {
    ...
    private final int initialNumberOfAccounts = 5; // Initial number of products
    ...
    public void testInsertAccount() throws DuplicateAccountException {
        ...
        String testUsername = "Inserted account";
        String testPassword = "KaasIsLekker";
        Integer testAccountType = 2;
        Account testAccount = new Account(testUsername, testPassword, testAccountType);
        ...
        // Try to fetch the product from the database. If it exists and ID is not the same as allready present in database, we have succesfully created a new product
        final String query = "SELECT * FROM `account` WHERE `id` NOT BETWEEN 1 and " + initialNumberOfAccounts + " AND `username`=? AND `password`=? AND `account_type`=?";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();) {
            PreparedStatement stat = connection.prepareStatement(query);
            stat.setString(1,testUsername);
            stat.setString(2,testPassword);
            stat.setString(3,testAccountType.toString());
    ...
    public void testInsertExistingAccount() {
        ...
        try {
            AccountDao accountDao = DaoFactory.getDaoFactory(DaoFactory.MYSQL).createAccountDao();
            accountDao.insertAccount(testAccount);
            fail("Adding an existing product should have thrown a DuplicateDatabaseEntryException");
    ...
    public void testDeleteAccount() {
        ...
        final String query = "SELECT * FROM `account` WHERE `id`=? AND `username`=? AND `password`=? AND `account_type`=?";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();) {
            PreparedStatement stat = connection.prepareStatement(query);
        ...
        final String queryForDeleted = "SELECT * FROM `account` WHERE `id`=? AND `name`=? AND `price`=? AND `stock`=?";
        try (Connection connection = DatabaseConnection.getInstance().getConnection();) {
